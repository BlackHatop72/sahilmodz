...
# Bot token
TOKEN = '7512318354:AAEuiH66Jrm35hMFQfL1xV7dNnbzvnkONck'
bot = telebot.TeleBot(TOKEN)
REQUEST_INTERVAL = 1

# Admins list
ADMIN_IDS = [1725783398]  # Replace with actual admin user IDs
...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*❌ You are not authorized to use this bot!*", parse_mode='Markdown')
...
def attack_command(message):
    ...
    try:
        bot.send_message(chat_id, "*❌ Enter this format: <ip> <port> <time>*", parse_mode='Markdown')
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*🚀 Attack send successfully 🚀 \n\n🔹 Host: {target_ip}\n🔌 Port: {target_port}\n⏳ Time: {duration} seconds\n✨ Method: Black Hat Antiban Injected\n\n🔥 Status: Attack in progress........*", parse_mode='Markdown')
    except Exception as e:
...
def reseller_command(message):
    try:
        # Get system status details
        response = "*Contact @BlackHatDDoS for reseller ship.*"
        bot.send_message(message.chat.id, response, parse_mode='Markdown')
    except Exception as e:
        logging.error(f"Error in status command: {e}")
...
def send_welcome(message):
    ...
    btn_reseller = KeyboardButton("/resellership")
    btn_info = KeyboardButton("myinfo")
    markup.row(btn_reseller, btn_info)

    bot.send_message(message.chat.id, "*🔥 Welcome user choose option:*", reply_markup=markup, parse_mode='Markdown')
